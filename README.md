...
**Refinement in progress, 2026-09-13:** the current saved place includes the ninety-minute extension checkpoint, independently scored **6.0/10** against the unchanged original target (composition 2.2, lighting 1.6, materials 1.6, detail 0.6). Actual screenshot and measurements are in `docs/iterations/03-90min.*` and `docs/extension-progress.md`. The 3600-frame stationary Studio sample averaged 60.0031 FPS at manual quality 10, 1920x1080; p95 17.7281 ms and p99 18.2777 ms. An earlier movement sample averaged 54.246 FPS with p99 67.07 ms, so stable moving performance is not yet established. Older validation sections below describe the previous run. Current editable working source is `Assets/Blender/Lumenwild_Consolidated90.blend`, preserving all 18 source scenes and 31 packed images, with incremental exports and installers under `Assets/Exports` and `tools/extension`.
